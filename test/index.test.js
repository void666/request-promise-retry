...
describe('request-promise-retry', function () {
    it('should  pass, with retry options', () => {
        return rp(optionsWithRetry)
            .then(data => {
                return Promise.resolve(expect(data.error).equal(undefined));
            });
    });
    ...
    it('fail and retry 3 times', () => {
        return rp(optionsWithRetryFail)
            .catch(err => {
                return Promise.resolve(expect(err.error.code).equal('ENOTFOUND'));
            });
    });
    ...
    it('should fail, negative retry option', () => {
        return rp(optionsBadRetry1)
            .catch(err => {
                return Promise.resolve(expect(err.message).equal('Retry count must be positive integer'));
            });
    });
    it('should fail, bad retry option', () => {
        return rp(optionsBadRetry2)
            .catch(err => {
                return Promise.resolve(expect(err.message).equal('Supports boolean or positive integer'));
            });
    });
    ...
    it('should pass, retry with exponential backoff', () => {
        ...
        return rp({
            ...
            return Promise.resolve(expect(new Date() - startTime).to.be.above(0 + 30 + 300 + 3000));
        });
    });
    it('should not retry, accepted options enabled', () => {
        return rp(optionsDontRetryAcceptedOptions)
            .catch(data => {
                return Promise.resolve(expect(data.accepted).equal(true));
            });
    });
